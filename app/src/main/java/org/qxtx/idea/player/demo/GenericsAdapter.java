...
public abstract class GenericsAdapter<T> extends RecyclerView.Adapter<GenericsAdapter<T>.MyHolder> {
    ...
    private MultiLayout mutilLayout;
    ...
    public GenericsAdapter(Context context, MultiLayout mutilLayout, List<T> data) {
        this.context = new WeakReference<>(context);
        this.mutilLayout = mutilLayout;
    ...
    /**
     * Return a item id from the position on the item list while {@link #mutilLayout} is not null,
    ...
    public int getItemViewType(int position) {
        if (mutilLayout != null) {
            return mutilLayout.getLayoutId(position);
        } else if (layoutId != -1) {
            return layoutId;
        }
    ...
    public long getItemId(int position) {
        return mutilLayout == null ? layoutId : mutilLayout.getLayoutId(position);
    }

    /**
     * If {@link #mutilLayout} is not null, it will be useless.
     * @param layoutId item id
     */
    public GenericsAdapter<T> setSingleLayoutId(int layoutId) {
        this.layoutId = layoutId;
        return this;
    }
     ...
     */
    public GenericsAdapter<T> setMultiLayout(MultiLayout mutilLayout) {
        this.mutilLayout = mutilLayout;
        return this;
    }
        ...
        public boolean setListener(int resId, Object listener) {
            ...
            switch (listenerName) {
                ...
                case InterfaceName.GENERIC_MOTION:
                    ...
                    break;
                default:
                    Log.e("lyx_tag", "MyHolder@setListener: Unknown listener.");
                    return false;
            }
