...
public class DefaultDrmSessionManager<T extends ExoMediaCrypto> implements DrmSessionManager<T>,
  ...
  // LYX_TODO: 2018/11/14 0014 待处理 应该被优化
  public Map<String, String> queryKeyStatus() {
    if (sessions == null || sessions.size() == 0) {
      return null;
    }
    return sessions.get(0).queryKeyStatus();
  }
  ...
  private static SchemeData getSchemeData(DrmInitData drmInitData, UUID uuid,
    ...
    for (int i = 0; i < drmInitData.schemeDataCount; i++) {
      ...
      boolean uuidMatches = schemeData.matches(uuid)/* || (C.CLEARKEY_UUID.equals(uuid))*/;
  ...
  private class MediaDrmHandler extends Handler {
    ...
    public void handleMessage(Message msg) {
      if (msg.obj instanceof String) {
        byte[] sessionId = ((String) msg.obj).split("&&")[0].getBytes();
        long expirationTime = Long.parseLong(((String) msg.obj).split("&&")[1]);
